...
class DECA(nn.Module):
    ...
    def displacement2normal(self, uv_z, coarse_verts, coarse_normals):
        ...
        torch_resize = Resize([cfg.model.uv_size,cfg.model.uv_size])
        im1_resize = torch_resize(self.fixed_uv_dis[None,None,:,:])
        fixed_uv_dis = torch.from_numpy(im1_resize.data.cpu().numpy()[0]).cuda()
    
        uv_z = uv_z*self.uv_face_eye_mask
        uv_detail_vertices = uv_coarse_vertices + uv_z*uv_coarse_normals + fixed_uv_dis*uv_coarse_normals.detach()
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        _, _, _, vertsT = self.flame(shape_params=codedict['shape'], expression_params=codedict['exp'], pose_params=codedict['pose'])
...
#        print(landmarks2d)
        ...
        if use_detail:
            ...
            torch_resize = Resize([cfg.model.uv_size,cfg.model.uv_size])
            im1_resize = torch_resize(uv_z)
            uv_z = torch.from_numpy(im1_resize.data.cpu().numpy()[0]).cuda()
            ...
            opdict['displacement_map'] = uv_z+fixed_uv_dis
...
#            print(uv_pverts.permute(0,2,3,1)[:,:,:,:2].shape)
#            texture_images = util.tensor2image(uv_gt[:,:3,:,:][0])
#            cv2.imwrite('TTTtexture_images.png', texture_images)
#            result1 = uv_pverts.permute(0,2,3,1)[:,:,:,:2].cpu().detach().numpy().reshape(1024, 2048)
#            np.savetxt('npresult1.txt',result1)
            texture_Photo = util.tensor2image((uv_gt[:,:3,:,:]*self.uv_face_eye_mask)[0])
            texture_3DMM = util.tensor2image(uv_texture[:,:3,:,:][0])
            cv2.imwrite('TTT1.png', texture_Photo)
            cv2.imwrite('TTT2.png', texture_3DMM)
            
            if cfg.model.uv_size == 256:
                mask_face = cv2.imread("D:\AuI18N\DECA-master2\demos\face_mask.jpg", 0)
            elif cfg.model.uv_size == 512:
                mask_face = cv2.imread("D:\AuI18N\DECA-master2\demos\face_mask_512.jpg", 0)
            elif cfg.model.uv_size ==1024: 
                mask_face = cv2.imread("D:\AuI18N\DECA-master2\demos\face_mask_1024.jpg", 0)
            
            if cfg.model.uv_size == 256:
                mask_eye = cv2.imread("D:\AuI18N\DECA-master2\demos\eye_mask.jpg", 0)
            elif cfg.model.uv_size == 512:
                mask_eye = cv2.imread("D:\AuI18N\DECA-master2\demos\eye_mask_512.jpg", 0)
            elif cfg.model.uv_size ==1024: 
                mask_eye = cv2.imread("D:\AuI18N\DECA-master2\demos\eye_mask_1024.jpg", 0)
                
            mask_eye_inverse = 255 - mask_eye
            
            texture_Photo_Eye=cv2.add(texture_Photo, np.zeros(np.shape(texture_Photo), dtype=np.uint8), mask=mask_eye)
            cv2.imwrite('TTT3.png', texture_Photo_Eye)
            
            width, height, channels = texture_3DMM.shape
            center = (int(height/2), int(width/2))

            PoissonImage = cv2.seamlessClone(texture_Photo, texture_3DMM, mask_face, center, cv2.NORMAL_CLONE)
            PoissonImageFace = cv2.add(PoissonImage, np.zeros(np.shape(PoissonImage), dtype=np.uint8), mask=mask_eye_inverse)
            PoissonImage = PoissonImageFace + texture_Photo_Eye
            
            PoissonImage = cv2.cvtColor(PoissonImage, cv2.COLOR_BGR2RGB)
            PoissonImageTensor = transforms.ToTensor()(PoissonImage)

            if self.cfg.model.use_tex:
                uv_texture_gt = PoissonImageTensor.cuda()
                uv_texture_gt = uv_texture_gt.unsqueeze(0)
            else:
            ...
            if self.cfg.model.use_tex:
                ...
                visdict['rendered_images_gt'] = ops_gt['images']
    ...
    def save_obj(self, filename, opdict):
        ...
        cv2.imwrite("TTTTTT.png", texture)
